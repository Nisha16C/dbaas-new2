<template>
  ...
  <div class="collapse navbar-collapse w-auto h-auto h-100" id="sidenav-collapse-main " style="margin-top:49px;">
    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item shift-right" v-if="checkuser(username)">

        <a href="#" @click="toggleDropdown('user-management')" class="nav-link">

          <i class="fa fa-users text-info text-sm opacity-10"></i>

          <span class="nav-link-text text-center" style="margin-right: 10px;">User Management</span>
      ...
      <li class="mt-3 nav-item" v-if="checkuser(username)" v-show="observabilityBackupVisible">
...
export default {
  ...
  methods: {
    ...
    fetchConnectedStatus() {
      return axios.get(`${this.apiUrl}/api/v1/is-connected/`)
        .then(response => {
          if (response.status === 200 && response.data) {
            return response.data.is_connected === 'True'; // Compare the string 'True'
          } else {
            console.error('Error fetching connected status');
            return false;
          }
        })
        .catch(error => {
    ...
    async redirectToADSave() {
      try {
        ...
        if (isConnected) {
          this.$router.push('/ADsave'); // Redirect to ADsave when isConnected is true
        } else {
          this.$router.push('/ADauthprovider'); // Redirect to ADauthprovider otherwise
        }
      } catch (error) {
