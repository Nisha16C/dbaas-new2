...
export default {
  ...
  methods: {
    ...
    Unschedule(serverName) {
      axios
        .post(`http://172.16.1.131:5000/barman/update-scheduled-backup?storage_method=nfs&server_name=${serverName}&remove_job=true`,)
        .then((response) => {
    ...
    changeRetention() {
      ...
      axios
        .post(
          `http://172.16.1.131:5000/barman/update-scheduled-backup?server_name=${this.server_name}&retention=${this.newRetentionPeriod}${this.selected_value}&storage_method=nfs`
        )
        .then((response) => {
