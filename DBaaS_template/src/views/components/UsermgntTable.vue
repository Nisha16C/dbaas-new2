  ...
  methods: {
    async prepareUserRole(user) {
      try {
        ...
        const response = await axios.get(`http://172.16.1.190:8002/api/v1/get_user_role/${this.selectedUser.id}/`);
    ...
    async fetchRoles() {
      try {
        const response = await axios.get(`http://172.16.1.190:8002/api/v1/users/`,
          {
    ...
    async assignRoles() {
      try {
        ...
        const response = await axios.post(`http://172.16.1.190:8002/api/v1/add_roles_to_user/`,
          {
            user_id: this.selectedUser.id,
