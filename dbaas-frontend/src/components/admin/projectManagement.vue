<template>
  ...
  <div class="container sm:ml-72">
    ...
    <div class="px-4 mt-10">
      <div class="mb-6">
        ...
        <input
          ...
          class="w-[300px] bg-gray-50 border border-gray-500 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-blue-900 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
    ...
    <div
      class="w-full h-15 border-b-2 p-4 border-solid mt-20 flex items-center"
    >
        
    <div class=" ">
      <div
        class="flex items-center px-5 py-6 bg-gray-100 rounded-md shadow-sm"
      >
          ...
          <table class="min-w-full">
                ...
                >
                  <div class="flex items-center">
                    <div class="flex-shrink-0 ">
                      <button 
                      class="flex mt-3 items-center justify-center h-8 w-8 bg-indigo-200 rounded-full">
                      {{ project.id }}
                    </button>
                    </div>
            ...
            >
              <span
                class="inline-flex px-2 text-xs font-semibold leading-5
                 text-yellow-500 bg-yellow-100 rounded-full"
                >{{ formatTimeAgo(project.created_date) }} </span
              >
            </td>
            <td
              class="px-6 py-4 border-b border-gray-200 whitespace-nowrap"
            >
              <span
                class="inline-flex px-2 text-xs font-semibold leading-5
                 text-yellow-500 bg-yellow-100 rounded-full"
                >{{ formatTimeAgo(project.updated_date) }} </span
              >
            </td>
            <td
                  class="px-6 py-4 text-sm font-medium leading-5 text-right border-b border-gray-200 whitespace-nowrap"
                >
                  <button @click.prevent="toggleModal(project.id, project.project_name)" class="text-indigo-600 hover:text-indigo-900"
                    > Edit </button>
      ...
      <div class="relative w-2/4">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-100">
          ...
          <div class="p-6">
            ...
            <button  to="Createuser"  @click.prevent="renameProject" class="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-600 to-blue-500 group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800">
              <span class="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
                Rename Project
              </span>
              </button>
...
export default {
  ...
  methods: {
    toggleModal(projectId, currentName) {
    ...
    saveProject() {
      ...
      if (this.input1Error) {
        ...
        axios
          .post("http://172.16.1.158:8002/api/v2/project/", this.project)
          .then((response) => {
            this.fetchProject();
    ...
    fetchProject() {
      ...
      axios
        .get(`http://172.16.1.158:8002/api/v2/project/`)
        .then((response) => {
    ...
    renameProject() {
      ...
      axios
        .put(
          `http://172.16.1.158:8002/api/v2/project/${this.renamingProjectId}/rename/`,
          payload
        )
        .then((response) => {
          // Handle success, e.g., close modal and refresh project list
          this.isModalVisible = !this.isModalVisible;
          this.fetchProject();
        })
        .catch((error) => {
