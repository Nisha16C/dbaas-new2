...
export default {
  ...
  methods: {
    checkClusterNameExists() {
        ...
        axios
        .get(`http://172.16.1.158:8002/api/v2/cluster/check_cluster_exists/?cluster_name=${this.cluster_name}&project_id=${this.project_id}`)
          .then((response) => {
    ...
    createCluster() {
      ...
      this.project_id = sessionStorage.getItem('project_id');
        ...
        axios
        .get(`http://172.16.1.158:8002/api/v3/providers/by-username-and-name/${this.Username}/${this.selectedProvider}/`)
        .then((response)=>{
          ...
          this.$router.push('/result');
          axios
            .post(`http://172.16.1.158:8002/api/v2/cluster/`, fromData)
            .then((response) => {
